...
MarinePID::MarinePID()
{
  ...
  m_max_thrust_force = 1;
  m_drag_coef        = 0;
  m_use_speed_pid  = true;
...
{
  m_iteration++;
  cout << "Marine PID iteration n° " << m_iteration << endl;
  postCharStatus();
  ...
  thrust = m_pengine.getDesiredThrust(m_desired_speed, m_current_speed, 
				    m_current_thrust, m_max_thrust, m_max_thrust_force, m_drag_coef, m_use_speed_pid);
  if(m_depth_control)
...
bool MarinePID::OnStartUp()
{
  ...
  for(p=sParams.begin(); p!=sParams.end(); p++) {
    ...
    else if(param == "MAX_THRUST_FORCE")
      m_max_thrust_force = dval;
    else if(param == "DRAG_COEF")
      m_drag_coef = dval;
    else if(param == "USE_SPEED_PID")
      setBooleanOnString(m_use_speed_pid, value);
  }
...
{
  int ok = true;
  //m_pengine.setSpeedFactor(m_speed_factor); //removed by Nathan
