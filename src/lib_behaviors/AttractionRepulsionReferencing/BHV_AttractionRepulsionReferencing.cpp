...
/*    NAME: Nick Rypkema                                    */
/*    ORGN: MIT                                             */
/*    FILE: BHV_AttractionRepulsionReferencing.cpp          */
/*    DATE:                                                 */
/************************************************************/
...
#include "BHV_AttractionRepulsionReferencing.h"
...
BHV_AttractionRepulsionReferencing::BHV_AttractionRepulsionReferencing(IvPDomain domain) :
  IvPBehavior(domain), BHV_DriftingForce(domain), BHV_AcousticPingPlanner(domain)
{
  // Provide a default behavior name
  IvPBehavior::setParam("name", "BHV_AttractionRepulsionReferencing");
...
bool BHV_AttractionRepulsionReferencing::setParam(string param, string val)
{
...
void BHV_AttractionRepulsionReferencing::onSetParamComplete()
{
...
void BHV_AttractionRepulsionReferencing::onHelmStart()
{
...
void BHV_AttractionRepulsionReferencing::onIdleState()
{
...
void BHV_AttractionRepulsionReferencing::onCompleteState()
{
...
void BHV_AttractionRepulsionReferencing::postConfigStatus()
{
...
void BHV_AttractionRepulsionReferencing::onIdleToRunState()
{
...
void BHV_AttractionRepulsionReferencing::onRunToIdleState()
{
...
IvPFunction* BHV_AttractionRepulsionReferencing::onRunState()
{
  ...
  for (map<string,PingContact>::iterator neighbour = m_contact_whitelist.begin(); neighbour != m_contact_whitelist.end(); ++neighbour) {  // loop through all neighbours in the whitelist
    ...
    if (m_display_targets) {
      ...
      m_abs_circle.setRad(m_zero_crossing);
      m_abs_circle.set_color("vertex", m_color);
      m_abs_circle.set_color("edge", m_color);
...
void BHV_AttractionRepulsionReferencing::attractionRepulsionAtomic(double dist, double heading, double& force_x, double& force_y)
{
  ...
  y = pow(dist,y_pow);
  a = 100.0;
  b = 100.0*pow(m_zero_crossing,diff_pow);
  force = -dist*(a/x - b/y);
  if (force < 0) {
    force = -force;
...
void BHV_AttractionRepulsionReferencing::attractionRepulsionPiecewiseLinear(double dist, double heading, double& force_x, double& force_y)
{
...
void BHV_AttractionRepulsionReferencing::postViewablePoints()
{
}
...
void BHV_AttractionRepulsionReferencing::postErasablePoints()
{
}
