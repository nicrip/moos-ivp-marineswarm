...
//
////  m_nodes_points = mat(3,2);
////  m_nodes_points(0,0) = 40.3345;
////  m_nodes_points(0,1) = 51.5784;
////  m_nodes_points(1,0) = -93.5976;
////  m_nodes_points(1,1) = 13.8248;
////  m_nodes_points(2,0) = 53.2631;
////  m_nodes_points(2,1) = -65.4032;
//
//  if (! iter_1) {
////    iter_1 = true;
//    vector<mat> scores;
...
//    Col<int> min_costs;
//    assignmentScores(m_nodes_points, 45, scores, assignments, min_costs);
...
//  m_warning_message << "MIN COSTS" << endl;
//  m_warning_message << min_costs << endl;
//  postWMessage(m_warning_message.str());
//  m_warning_message.clear();
//  m_warning_message.str("");
  mat optimal_formation;
  optimalAssignment(scores, assignments, min_costs, optimal_formation);
//  m_warning_message << "optimal_formation" << endl;
//  m_warning_message << optimal_formation << endl;
//  postWMessage(m_warning_message.str());
//  m_warning_message.clear();
//  m_warning_message.str("");
//  m_warning_message << "m_nodes_points" << endl;
//  m_warning_message << m_nodes_points << endl;
//  postWMessage(m_warning_message.str());
//  m_warning_message.clear();
//  m_warning_message.str("");
...
//  m_warning_message.str("");
  ...
  for (unsigned int j = 0; j < m_sorted_formation_points.size(); j++) { //m_sorted_formation_points.size()
    ...
    /****/
//    m_warning_message << "NODES " << j << endl;
//    m_warning_message << nodes_offset << endl;
//    postWMessage(m_warning_message.str());
//    m_warning_message.clear();
//    m_warning_message.str("");
    /****/
...
//    m_warning_message.str("");
    ...
    for (unsigned int i = 0; i < num_rot; i++) { //num_rot
      ...
      /****/
//      m_warning_message << "COST MATRIX " << j << i << endl;
...
//      m_warning_message << (i+1)*angle_delta << endl << trans(costs);
      ...
      hungarian_problem_t hungarian;
      int matrix_size = hungarian_init(&hungarian, m , cols, rows, HUNGARIAN_MODE_MINIMIZE_COST);
      hungarian_solve(&hungarian);
      if (hungarian.opt_cost < min_cost) {
        min_cost = hungarian.opt_cost;
        min_assignment = hungarian.opt_assignment;
...
//      hungarian_free(&hungarian);
      ...
      /****/
//      m_warning_message << "COST " << j << " - " << m_hungarian.opt_cost << endl;
//      m_warning_message << "[";
...
/**MUNKRES ASSIGNMENT ALGORITHM START**/
