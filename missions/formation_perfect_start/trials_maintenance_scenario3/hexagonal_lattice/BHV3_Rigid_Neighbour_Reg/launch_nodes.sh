...
do
  ...
  -h | --help | -\?)
    ...
    printf "  --offsetAUVs=value : number of AUVs in the mission, default is $SWARM_OFFSET \n\t(shortcuts: -o) \n"
  ...
  -o=* | --offsetAUVs=*)
    SWARM_OFFSET=${1#*=}
    printf "\tNew parameter SWARM_OFFSET: %d\n" $SWARM_OFFSET
    shift
    ;;
  -w=* | --width=*)
    RANDOM_BOX_WIDTH=${1#*=}
  ...
  do
    for ((j = 0; j < $SWARM_NUM_NODES_Y; j++))
      do
        ...
        X_TOTAL=$(echo "$X_TOTAL+$x_pos" | bc -l)
        Y_TOTAL=$(echo "$Y_TOTAL+$y_pos" | bc -l)
        COUNT=$((COUNT+1))
    done
done
X_TOTAL=$(echo "$X_TOTAL / $COUNT" | bc -l)
Y_TOTAL=$(echo "$Y_TOTAL / $COUNT" | bc -l)
echo $X_TOTAL
echo $Y_TOTAL
  ...
  do
    for ((j = 0; j < $SWARM_NUM_NODES_Y; j++))
      do
        ...
        x_pos=$(echo "$x_pos-$X_TOTAL" | bc -l)
        y_pos=$(echo "$y_pos-$Y_TOTAL" | bc -l)
