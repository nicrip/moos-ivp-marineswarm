...
#cd src/iMatlab
#bash /opt/R2013b/bin/matlab -nodisplay -r make_imatlab
#cd ../
...
for ARGI; do
	...
	exit 0;
        ...
        BUILD_TYPE="Release"
    elif [ "${ARGI}" = "--no-gui" ]; then
    	BUILD_GUI=OFF
   	elif [ "${ARGI}" = "--w-gui" ]; then
    	BUILD_GUI=ON
    else
...
cmake -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DBUILD_GUI_CODE=${BUILD_GUI} ../
